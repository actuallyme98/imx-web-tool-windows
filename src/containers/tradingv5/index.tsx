...
const TradingV5Page: React.FC = () => {
  ...
  const onPreCreateOrders = async (selectedClient: TradingServiceV3) => {
    ...
    let startPrice = parseFloat(startingPrice);
    let endPrice = parseFloat(endingPrice);
    ...
    for (const client of clients) {
      try {
        ...
        startPrice -= parseFloat(diffPrice);
        endPrice += parseFloat(diffPrice);
      } catch (error: any) {
        pushLog(fileName, {
          title: error.message,
          type: 'error',
  ...
  const tradingv5 = async (fileAndClient: TradingServiceV3) => {
  ...
  const onStartTrade = async () => {
    ...
    try {
      ...
      for (const fileAndClient of fileAndClients) {
        promises.push(tradingv5(fileAndClient));
      }
