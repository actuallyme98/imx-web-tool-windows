...
// utils
import { fromCsvToUsers, toIMXAmount } from '../../utils/format.util';
import { delay } from '../../utils/system';
import { getIMXPrice } from '../../utils/api.util';
...
const TradingV7Page: React.FC = () => {
  ...
  const [tgasLimit, setTGasLimit] = useState('100000');

  const [smaxFeePerGas, setSMaxFeePerGas] = useState('10');
  const [smaxPriorityFeePerGas, setSMaxPriorityFeePerGas] = useState('10');
  const [sgasLimit, setSGasLimit] = useState('100000');
  ...
  const sellAmount = useMemo(
    () => String(parseFloat(endingPrice) + parseFloat(startingPrice)),
  ...
  const onChangeStartingPrice = (event: React.ChangeEvent<HTMLInputElement>) => {
    const value = event.target.value;
    setStartingPrice(value);
  };
  const onChangeEndingPrice = (event: React.ChangeEvent<HTMLInputElement>) => {
    const value = event.target.value;
    setEndingPrice(value);
  };
  const onChangeDiffPrice = (event: React.ChangeEvent<HTMLInputElement>) => {
    const value = event.target.value;
    setDiffPrice(value);
  };
  ...
  const onLoadWallets = async (imxPrice: number) => {
    ...
    for (const client of updatedClients) {
      const fullFillClients = await onPreCreateOrders(client, imxPrice);
    ...
    return updatedStateFileAndClient;
  };

  const onPreCreateOrders = async (selectedClient: TradingServiceV3, imxPrice: number) => {
    ...
    let startPrice = toIMXAmount(parseFloat(startingPrice), imxPrice);
    let endPrice = toIMXAmount(parseFloat(endingPrice), imxPrice);
    const toDiffPrice = toIMXAmount(parseFloat(diffPrice), imxPrice);

    pushLog(fileName, {
      title: `Selling with prices ${startPrice} & ${endPrice}!`,
    });

    for (const client of clients) {
      try {
        const { service, tokenAddress, tokenAddress2, tokenId, tokenId2 } = client;
        ...
        if (!tokenAddress || !tokenId || !tokenId2) {
          pushLog(fileName, {
        ...
        const gasOptions = {
          maxPriorityFeePerGas: smaxPriorityFeePerGas
            ...
            : 10e9,
          maxFeePerGas: smaxFeePerGas ? parseFloat(smaxFeePerGas) * 1e9 : 10e9,
          gasLimit: sgasLimit ? parseFloat(sgasLimit) : 100000,
        };
        ...
        const createdOrderResponse = await service.sell(
          {
            request: {
              buy: {
                amount: etherToWei(String(startPrice)),
                type: 'ERC20',
        ...
        const createdOrderResponse2 = await service.sell(
          {
            request: {
              buy: {
                amount: etherToWei(String(endPrice)),
                type: 'ERC20',
                tokenAddress: IMX_ADDRESS,
              },
              sell: {
                tokenAddress: tokenAddress2 || tokenAddress,
                tokenId: tokenId2,
                type: 'ERC721',
              },
            },
        ...
        pushLog(fileName, {
          title: `Created order success with order id ${createdOrderResponse2.order_id}`,
          type: 'success',
        });
        ...
        updatedClients.push({
          ...
          orderId2: String(createdOrderResponse2.order_id),
        });

        startPrice -= toDiffPrice;
        endPrice += toDiffPrice;
      } catch (error: any) {
        pushLog(fileName, {
          title: error.message,
          type: 'error',
  ...
  const triggerTransfer = async (
    ...
    const IMXSellAmount = toIMXAmount(parseFloat(sellAmount), imxPrice);

    const minRequiredBalance = IMXSellAmount;

    let retryAttempts = 0;

    pushLog(fileName, {
      title: `Starting transfer ${minRequiredBalance} IMX from ${ethAddress}`,
    });
    ...
    const gasOptions = {
      ...
      gasLimit: tgasLimit ? parseFloat(tgasLimit) : 100000,
    };
    ...
    while (retryAttempts < retryCount) {
      try {
        await service.transfer(
          {
            request: {
              ...
              amount: etherToWei(String(IMXSellAmount)),
              tokenAddress: '',
            },
          },
          gasOptions,
        ...
        const resp = await service.getBalance();
        pushLog(fileName, {
          title: `${ethAddress} has balance of ${resp.balance}`,
        });
  ...
  const tradingv7 = async (fileAndClient: TradingServiceV3, imxPrice: number) => {
    ...
    if (!clients.length || !rootPrivateKey) {
      pushLog(fileName, {
    ...
    const poolService = new ImmutableService('imxZkEVM', rootPrivateKey, '');
    const rootAddress = poolService.getAddress();
    const firstAddress = rootWallet.service.getAddress();

    await triggerTransfer(poolService, firstAddress, 50, fileName, imxPrice);
    await delay(1500);
    // let retryAttempts = 0;

    for (const selectedClient of restClients) {
      // while (retryAttempts < 50) {
      try {
        const { service, orderId, orderId2, index } = selectedClient;
        const ethAddress = service.getAddress();
        pushLog(fileName, {
          title: `Selected Address: ${ethAddress}`,
        });
        ...
        if (index % 2 === 0) {
          if (!orderId || !orderId2) {
            pushLog(fileName, {
              title: 'Skip this address because orderId or orderId2 is empty',
            });
            continue;
          }
          pushLog(fileName, {
            title: 'Trigger buy order 1!',
          });
          await triggerBuy(rootWallet, orderId, 50, fileName);
          pushLog(fileName, {
            title: 'Trigger buy order 2!',
          });
          await triggerBuy(rootWallet, orderId2, 50, fileName);

          poolWallet = selectedClient;
        } else {
          await delay(1500);
          await triggerTransfer(poolWallet.service, ethAddress, 50, fileName, imxPrice);
    ...
    await triggerTransfer(poolWallet.service, rootAddress, 50, fileName, imxPrice);
  };

  const onStartTrade = async () => {
    setIsTradeSubmitting(true);

    let selectedFileAndClients = fileAndClients;

    let imxPrice = 0;

    if (tradingTime) {
      const now = moment();
      const tradingMoment = moment(tradingTime);

      const diffTime = moment(tradingMoment).diff(now);

      const TEN_SECONDS = 10000;

      const waitingTime = diffTime - TEN_SECONDS;

      if (waitingTime > 0) {
        await delay(waitingTime);
      }

      imxPrice = await getIMXPrice();

      pushLog('[SYSTEM]', {
        title: `IMX Price is ${imxPrice}`,
        type: 'error',
      });

      const updatedStateFileAndClient = await onLoadWallets(imxPrice);
      selectedFileAndClients = updatedStateFileAndClient;
    }
    ...
    try {
      ...
      for (const fileAndClient of selectedFileAndClients) {
        promises.push(tradingv7(fileAndClient, imxPrice));
      }
          ...
          <Grid item xs={8}>
            ...
            <Box>
              <SubmitButton
                ...
                disabled={isTradeSubmitting || !startingPrice || !endingPrice || !diffPrice}
              >
                Start trade
              </SubmitButton>
